...
public class UserControllerIT {
    ...
    UserEntity testUser, testAdmin, testEmployee;
    ...
    @BeforeEach
    void setUp() {
        testUser = testDataUtils.createTestUser("user@user.com", "testUser");
        testAdmin = testDataUtils.createTestAdmin("admin@admin.com", "adminTest");
        testEmployee = testDataUtils.createTestEmployee("employee@worker.com", "employeeTest");
    }
    @AfterEach
    void tearDown() {
    ...
    @Test
    @WithMockUser(username = "employeeTest", roles = {"EMPLOYEE","USER"})
    ...
    void testRemovingRoleWorksCorrectly() throws Exception {
        mockMvc.perform(patch("/users/roles/remove/{id}", testEmployee.getId()).with(csrf()))
                .andExpect(status().is3xxRedirection())
                .andExpect(redirectedUrl("/users/change/" + testEmployee.getId().toString()));
